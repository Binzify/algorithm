...
def mst_prim(graph, s):
    ...
    while connect < V:  # 모든 노드를 다 방문할 때까지 (최소신장트리는 nodes-1개의 간선을 가질때까지 반복되므로)
        for next_node in range(nodes):  # 노드의 개수만큼 for문을 돌려서 다음 이동할 노드의 가중치 확인
            if (
                visited[next_node] == 0 and graph[s][next_node] < distance[next_node]
            ):  # 다음 노드를 방문하지 않았고, 가중치가 작다면
                distance[next_node] = graph[s][
                    next_node
                ]  # 해당 가중치 입력 리스트에 그래프에 입력되어 있는 가중치를 입력한다.

        # 최소 가중치 값 입력해 둔 후 다음 노드를 돌 때 확인하기
        short_dis = INF  # 가장 작은 거리를 무한대로 우선 놔두고
        for next_node in range(nodes):  # 다시한번 노드를 순회하면서
            if (
                visited[next_node] == 0 and distance[next_node] < short_dis
            ):  # 위에서 입력된 가중치가 작은 값이라면
...
for tc in range(1, T + 1):
    ...
    graph = [
        [INF] * nodes for _ in range(nodes)
    ]  # 마지막 노드번호 +1개만큼 노드 생성 [[9999, 1, 1], [1, 9999, 6], [1, 6, 9999]]
