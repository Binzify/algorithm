...
N = int(input())  # 스위치의 개수
switch = [2] + list(map(int, input().split()))  # 받는 숫자랑 스위치 위치 숫자랑 동일하게 만들기 위해 앞에 연관없는 1개의 수를 넣음

students = int(input())  # 스위치 조작할 학생들의 수
for stud in range(students):
    gender, number = map(int, input().split())
    if gender == 1 :
        for i in range(number, N+1, number):  # 남자는 배수로 가는데, 주어진 수를 리스트범위(N개 +1내가 추가한)만큼 배수대로 감
            onoff(i)
    else:  # 여자일 때 여자는 받은 번호 위치의 -1과 +1이 같으면 스위치를 바꾸고 그렇지 않으면 받은 번호만 바꾼다.
        onoff(number)
        for k in range(1, N):
            if switch[number-k] == switch[number+k] and number-k >= 0 and number+k < N+1:
                onoff(number-k)
                onoff(number+k)
